...
function vendor() {
	return src(["src/js/**/*", "!src/js/script.js", "!src/js/lang/**/*"])
...
function html() {
	return src("src/*.html") // run the Twig template parser on all .html files in the "src" directory
		.pipe(
			$.twig({ data: { modified_time: new Date().getTime() } }).on(
				"error",
				console.log
			)
		)
...
function watch_file() {
	watch("src/sass/*", parallel(css, html));
...
exports.build = series(
	clean,
	parallel(css, vendor, js, copy_images, copy_js, copy_fonts, html)
);
exports.clean = clean;
